...
Log each complete step in log file with a terse summary note (one or two short sentences). Include a starting timestamp before you begin, and an ending timestamp when you are finished with all tasks. Timestamp should include full date and local hour minute second, obtainable from the local machine. In San Francisco this is Pacific / Los Angeles (PDT/PST). 
...
macOS note: on a fresh Mac, `git` is provided by the Xcode Command Line
Tools. First run `git --version` — if the tools are missing, macOS opens a
dialog offering to install them. Tell the developer to accept the dialog,
wait for the install to finish, then re-run `git --version` before cloning.
(The tools are also required later when `npm install` compiles native
modules, so this is worth confirming now.)
