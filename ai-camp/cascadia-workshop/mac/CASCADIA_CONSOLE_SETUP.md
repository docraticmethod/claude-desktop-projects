...
- Node setup: install Node 24, `npm init`, `npm install`. (The README's
  NodeSource instructions are Linux-only; on macOS install Node 24 via
  Homebrew instead — see Step 6.)
...
Illustrative only (a Mac workstation — yours will differ; do not paste):
   TARGET_WORKFLOW=/Users/<user>/claude-desktop-projects/ai-camp/cascadia-harness-node/context-docs/workflow
   TARGET_TEMPLATE=/Users/<user>/claude-desktop-projects/ai-camp/cascadia-harness-node/context-docs/template
...
(If the Cascadia Harness setup was just completed on this machine, Node 24
and the Xcode Command Line Tools will already be present and this step is a
quick confirmation.)
...
(`node@24` is a versioned formula, so the explicit `brew link` is required
to put `node` and `npm` on the PATH.)

If Homebrew itself is not installed (`brew --version` fails), STOP and ask
the developer to choose one of these two options — do not attempt either
non-interactively yourself:

- Install Homebrew from https://brew.sh (its installer prompts for an admin
  password, so the developer must run it themselves in Terminal), then
  return here and re-run this step; or
- Download and run the official Node 24 macOS installer (.pkg) from
  https://nodejs.org, then return here and re-run this step.

Also confirm the Xcode Command Line Tools are present — Step 8's
`npm install` compiles the native `better-sqlite3` module and fails
without them:
...
If that prints a path (e.g. `/Library/Developer/CommandLineTools`), the
tools are present. If it errors, run `xcode-select --install`, tell the
developer a macOS dialog will appear asking them to confirm the install,
